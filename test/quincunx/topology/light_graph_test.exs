defmodule Quincunx.Topology.LightGraphTest do
  use ExUnit.Case

  alias Quincunx.Topology.LightGraph
  ...
  describe "add_node/2" do
    test "adds a single node" do
      g = LightGraph.new() |> LightGraph.add_node(:a)
  ...
  describe "remove_node/2" do
    setup do
      g = LightGraph.new()
          |> LightGraph.add_node(:a)
          |> LightGraph.add_node(:b)
          |> LightGraph.add_node(:c)
          |> LightGraph.add_edge(:a, :b)
          |> LightGraph.add_edge(:b, :c)
      {:ok, graph: g}
    end

    test "removes an isolated node", %{graph: g} do
      g2 = LightGraph.remove_node(g, :c)
  ...
  describe "add_edge/2" do
    ...
    test "adding edge where nodes are missing produces inconsistent state" do
      g = LightGraph.new()
      g2 = LightGraph.add_edge(g, :missing, :also_missing)
  ...
  describe "remove_edge/2" do
    ...
    test "removes an existing edge", %{graph: g} do
      g2 = LightGraph.remove_edge(g, :a, :b)
    ...
    test "removing a non-existent edge does nothing", %{graph: g} do
      g2 = LightGraph.remove_edge(g, :b, :a)
      assert g2 == g
    end

    test "removing only one instance when duplicate edges exist", %{graph: g} do
      g2 = g
          |> LightGraph.add_edge(:a, :b)   # duplicate
          |> LightGraph.remove_edge(:a, :b)
  ...
  describe "topological_sort/1" do
    test "returns empty list for empty graph" do
      assert LightGraph.topological_sort(LightGraph.new()) == {:ok, []}
    end

    test "returns single node for graph with one node" do
      g = LightGraph.new() |> LightGraph.add_node(:a)
      assert LightGraph.topological_sort(g) == {:ok, [:a]}
    end
    ...
    test "orders nodes correctly in a DAG" do
      ...
      assert LightGraph.topological_sort(g) == {:ok, [:a, :b, :c]}
    end
    ...
    test "handles multiple roots" do
      g = LightGraph.new()
          ...
          |> LightGraph.add_node(:b)
          |> LightGraph.add_node(:c)
          |> LightGraph.add_edge(:a, :c)
          |> LightGraph.add_edge(:b, :c)

      {:ok, order} = LightGraph.topological_sort(g)
    ...
    test "detects a simple cycle" do
      g = LightGraph.new()
          ...
          |> LightGraph.add_node(:b)
          |> LightGraph.add_edge(:a, :b)
          |> LightGraph.add_edge(:b, :a)

      assert LightGraph.topological_sort(g) == {:error, :cycle_detected}
    end
    ...
    test "duplicate edges affect indegree count and break topological sort" do
      ...
      assert LightGraph.topological_sort(g) == {:ok, [:a, :b]}
    end
  end
  ...
  describe "dependents/2" do
    test "returns empty set for node with no outgoing edges" do
      g = LightGraph.new() |> LightGraph.add_node(:a)
      assert LightGraph.dependents(g, :a) == MapSet.new()
    end
    ...
    test "returns direct and transitive downstream nodes" do
      g = LightGraph.new()
          ...
          |> LightGraph.add_node(:b)
          |> LightGraph.add_node(:c)
          |> LightGraph.add_node(:d)
          |> LightGraph.add_edge(:a, :b)
          |> LightGraph.add_edge(:b, :c)
          |> LightGraph.add_edge(:a, :d)

      assert LightGraph.dependents(g, :a) == MapSet.new([:b, :c, :d])
    end

    test "excludes the node itself" do
      g = LightGraph.new()
          |> LightGraph.add_node(:a)
          |> LightGraph.add_edge(:a, :a)   # self-loop (allowed by data model)
    ...
    test "handles diamond-shaped DAG" do
      g = LightGraph.new()
          ...
          |> LightGraph.add_edge(:a, :b)
          |> LightGraph.add_edge(:a, :c)
          |> LightGraph.add_edge(:b, :d)
          |> LightGraph.add_edge(:c, :d)
  ...
  describe "dependencies/2" do
    test "returns empty set for node with no incoming edges" do
      g = LightGraph.new() |> LightGraph.add_node(:a)
      assert LightGraph.dependencies(g, :a) == MapSet.new()
    end
    ...
    test "returns direct and transitive upstream nodes" do
      ...
      assert LightGraph.dependencies(g, :c) == MapSet.new([:a, :b])
    end

    test "excludes the node itself" do
      g = LightGraph.new()
          |> LightGraph.add_node(:a)
          |> LightGraph.add_edge(:a, :a)
      assert LightGraph.dependencies(g, :a) == MapSet.new()
    end
  end
  ...
  describe "roots/1" do
    ...
    test "returns all nodes when graph has no edges" do
      g = LightGraph.new()
          |> LightGraph.add_node(:a)
          |> LightGraph.add_node(:b)
      assert Enum.sort(LightGraph.roots(g)) == [:a, :b]
    end
  end
  ...
  describe "leaves/1" do
    ...
    test "returns all nodes when graph has no edges" do
      g = LightGraph.new()
          |> LightGraph.add_node(:a)
          |> LightGraph.add_node(:b)
      assert Enum.sort(LightGraph.leaves(g)) == [:a, :b]
    end
  end
end
